...
VOID ComputeBoundingOrientedBoxFromPoints( OrientedBox* pOut, UINT Count, const XMFLOAT3* pPoints, UINT Stride )
{
    ...
    // Find the minimum OBUFFER_BIND using the eigenvectors as the axes.
    XMVECTOR vMin, vMax;
...
BOOL IntersectTriangleTriangle( FXMVECTOR A0, FXMVECTOR A1, FXMVECTOR A2, CXMVECTOR B0, CXMVECTOR B1, CXMVECTOR B2 )
{
    ...
    XMVECTOR BUFFER_BIND0, BUFFER_BIND1, BUFFER_BIND2;
    bool bPositiveB;
        ...
        XMVector3AllTrue( XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select0111 ) ) )
    {
        ...
        BUFFER_BIND0 = B0; BUFFER_BIND1 = B1; BUFFER_BIND2 = B2;
        bPositiveB = true;
    }
             ...
             XMVector3AllTrue( XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select0111 ) ) )
    {
        ...
        BUFFER_BIND0 = B0; BUFFER_BIND1 = B2; BUFFER_BIND2 = B1;
        bPositiveB = false;
    }
             ...
             XMVector3AllTrue( XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select1011 ) ) )
    {
        ...
        BUFFER_BIND0 = B1; BUFFER_BIND1 = B2; BUFFER_BIND2 = B0;
        bPositiveB = true;
    }
             ...
             XMVector3AllTrue( XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select1011 ) ) )
    {
        ...
        BUFFER_BIND0 = B1; BUFFER_BIND1 = B0; BUFFER_BIND2 = B2;
        bPositiveB = false;
    }
             ...
             XMVector3AllTrue( XMVectorSelect( BDistIsGreater, BDistIsLessEqual, Select1101 ) ) )
    {
        ...
        BUFFER_BIND0 = B2; BUFFER_BIND1 = B0; BUFFER_BIND2 = B1;
        bPositiveB = true;
    }
             ...
             XMVector3AllTrue( XMVectorSelect( BDistIsLess, BDistIsGreaterEqual, Select1101 ) ) )
    {
        ...
        BUFFER_BIND0 = B2; BUFFER_BIND1 = B1; BUFFER_BIND2 = B0;
        bPositiveB = false;
    }
    else
    {
    ...
    {
        Delta0 = ( BUFFER_BIND0 - AA0 );
        Delta1 = ( AA0 - BUFFER_BIND0 );
    }
    else
    {
        Delta0 = ( AA0 - BUFFER_BIND0 );
        Delta1 = ( BUFFER_BIND0 - AA0 );
    }
    ...
    XMVECTOR Dist0 = XMVector3Dot( Delta0, XMVector3Cross( ( BUFFER_BIND2 - BUFFER_BIND0 ), ( AA2 - AA0 ) ) );
    if( XMVector4Greater( Dist0, Zero ) )
        return FALSE;

    XMVECTOR Dist1 = XMVector3Dot( Delta1, XMVector3Cross( ( BUFFER_BIND1 - BUFFER_BIND0 ), ( AA1 - AA0 ) ) );
...
BOOL IntersectAxisAlignedBoxOrientedBox( const AxisAlignedBox* pVolumeA, const OrientedBox* pVolumeB )
{
    ...
    // Make the axis aligned box oriented and do an OBUFFER_BIND vs OBUFFER_BIND test.
    OrientedBox BoxA;
...
INT IntersectAxisAlignedBoxFrustum( const AxisAlignedBox* pVolumeA, const Frustum* pVolumeB )
{
    ...
    // Make the axis aligned box oriented and do an OBUFFER_BIND vs frustum test.
    OrientedBox BoxA;
	...
	auto ret = IntersectOrientedBoxFrustum(&BoxA, pVolumeB);
	return ret;
}

inline float Dot(const XMFLOAT4 & v0, const XMFLOAT4 & v1)
{
	return v0.x * v1.x + v0.y * v1.y + v0.z * v1.z + v0.w * v1.w;
}

INT IntersectAxisAlignedBoxFrustum2(const AxisAlignedBox* pVolumeA, const std::vector<XMFLOAT4> * frustumPlanes)
{
	XMASSERT(pVolumeA);
	//XMASSERT(pVolumeB);

	XMFLOAT3 aabbMin;
	aabbMin.x = pVolumeA->Center.x - pVolumeA->Extents.x;
	aabbMin.y = pVolumeA->Center.y - pVolumeA->Extents.y;
	aabbMin.z = pVolumeA->Center.z - pVolumeA->Extents.z;

	XMFLOAT3 aabbMax;
	aabbMax.x = pVolumeA->Center.x + pVolumeA->Extents.x;
	aabbMax.y = pVolumeA->Center.y + pVolumeA->Extents.y;
	aabbMax.z = pVolumeA->Center.z + pVolumeA->Extents.z;

	//std::vector<XMVECTOR> planes(6);
	//ComputePlanesFromFrustum(pVolumeB, &planes[0], &planes[1], &planes[2], &planes[3], &planes[4], &planes[5]);
	...
	for (int i = 0; i < 6; ++i)
	{
		auto & plane = planes[i];
		//XMFLOAT4 plane;
		//XMStoreFloat4(&plane, p);
		// v1 is diagonally opposed to v0
		XMFLOAT4 v0((plane.x > 0) ? aabbMin.x : aabbMax.x, (plane.y > 0) ? aabbMin.y : aabbMax.y, (plane.z > 0) ? aabbMin.z : aabbMax.z, 1.0f);
		XMFLOAT4 v1((plane.x > 0) ? aabbMax.x : aabbMin.x, (plane.y > 0) ? aabbMax.y : aabbMin.y, (plane.z > 0) ? aabbMax.z : aabbMin.z, 1.0f);
		//XMVECTOR v0XM = XMLoadFloat4(&v0);
		//XMVECTOR v1XM = XMLoadFloat4(&v1);
		...
		//if (XMVectorGetX(XMVector4Dot(p, v0XM)) > 0)
		//{
		//	return 0;
		//}
		//if (XMVectorGetX(XMVector4Dot(p, v1XM)) > 0)
		//{
		//	intersect = true;
		//}
